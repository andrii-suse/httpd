...
curl $(ap1/print_address.sh)/folder1?F=2
curl $(ap1/print_address.sh)/folder1?F=2 | grep -C10 file1.dat | grep file2.dat
# must use sed, otherwise observe line 8 column 3 - Warning: <table> lacks "summary" attribute 
curl $(ap1/print_address.sh)/folder1?F=2 | sed 's^<table>^<table summary="">^' | tidy -quiet -errors
