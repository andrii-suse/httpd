...
dockerfile=${ENVIRONS_DOCKERFILE:-$thisdir/Dockerfile.environs.opensuse.leap}
# dockerfile=${ENVIRONS_DOCKERFILE:-$thisdir/Dockerfile.environs.ubuntu}
basename=$(basename "$testcase")
basename=${basename,,}
basename=${basename//:/_}
ident=apache.envtest
...
echo "$*"
[ -z $initscript ] || echo "bash -xe /opt/environs/httpd/test/integration/$initscript" | docker exec -i "$containername" bash -x
